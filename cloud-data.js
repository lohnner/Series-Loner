...
function publishRanking(snapshot){const users=[],seriesScores={};snapshot.forEach(item=>{const data=item.data();users.push({uid:item.id,name:data.name||'Usuário',xp:Number(data.xp)||0,watchlistCount:Number(data.watchlistCount)||data.watchlist?.length||0});(data.watchlist||[]).forEach(key=>seriesScores[key]=(seriesScores[key]||0)+1)});users.sort((a,b)=>b.xp-a.xp||a.name.localeCompare(b.name));window.dispatchEvent(new CustomEvent('seriesloner-ranking-change',{detail:{users,seriesScores}}))}
