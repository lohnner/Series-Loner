...
function publishRanking(snapshot){const users=[],seriesScores={};snapshot.forEach(item=>{const data=item.data();users.push({uid:item.id,name:data.name||'Usuário',xp:Number(data.xp)||0});(data.watchlist||[]).forEach(key=>seriesScores[key]=(seriesScores[key]||0)+1)});users.sort((a,b)=>b.xp-a.xp||a.name.localeCompare(b.name));window.dispatchEvent(new CustomEvent('seriesloner-ranking-change',{detail:{users,seriesScores}}))}
async function writeAll(){const user=activeUser,progress=localProgress(),stats=totals(progress),name=user.displayName||'Usuário';await Promise.all([setDoc(doc(db,'users',user.uid),{name,progress,ratings:localRatings(user.uid),...stats,updatedAt:serverTimestamp()},{merge:true}),setDoc(doc(db,'publicProfiles',user.uid),{name,xp:stats.xp,watchlist:Object.entries(progress).filter(([,state])=>state?.inWatchlist).map(([key])=>key),updatedAt:serverTimestamp()},{merge:true})]);if(activeUser?.uid===user.uid)sessionStorage.removeItem(DIRTY_KEY)}
