...
function renderChannelRanking(scores){const board=document.getElementById('channel-ranking-board'),rows=Object.entries(scores).filter(([,score])=>score>0).sort((a,b)=>b[1]-a[1]||a[0].localeCompare(b[0],'pt-BR'));board.innerHTML=rows.length?rows.map(([channel,score],index)=>`<article class="rank-row channel-rank-row ${index===0?'rank-first':''}"><strong class="rank-position">${String(index+1).padStart(2,'0')}</strong><span class="channel-mark">${escapeChannel(initials(channel))}</span><div class="rank-name"><small>${index===0?'CANAL LÍDER':'NO RANKING'}</small><h2>${escapeChannel(channel)}</h2><p>${score} ${score===1?'série adicionada':'séries adicionadas'}</p></div><div class="rank-score"><strong>${score}</strong><span>${score===1?'ponto':'pontos'}</span></div></article>`).join(''):'<div class="calendar-empty">O ranking aparecerá quando uma série for adicionada à lista.</div>'}
