(async()=>{
  ...
  const card=(item,compact=false)=>`<article class="catalog-card ${compact?'featured-card':''}" data-id="${item.id}"><a class="catalog-poster" href="${encodeURI(item.page)}"><img src="${encodeURI(item.image)}" alt="Capa de ${item.title}" loading="lazy"><span>${item.status}</span></a><div class="catalog-info"><small>${item.year} · <a class="catalog-channel-link" href="canal.html?canal=${encodeURIComponent(item.channel||item.platform)}">${item.channel||item.platform}</a></small><h2><a href="${encodeURI(item.page)}">${item.title}</a></h2><p>${item.genres.join(' · ')}</p><div class="catalog-card-meta"><span>★ ${item.rating||'—'}</span><span>${item.seasons} temporada${item.seasons===1?'':'s'}</span><span>${item.country}</span></div></div></article>`;
