(async()=>{
  ...
  const response=await fetch('series-data.json?v=44');
  ...
  const controls=document.createElement('section');controls.className='catalog-tools';controls.innerHTML=`
    ...
    </div><fieldset class="genre-filter"><legend>Gêneros <small>(selecione um ou mais)</small></legend>${genres.map(genre=>`<label><input type="checkbox" value="${genre}"><span>${genre}</span></label>`).join('')}</fieldset><button id="clear-filters" type="button">Limpar filtros</button></details>
  ...
  const featured=document.createElement('section');featured.className='catalog-featured';featured.innerHTML='<div class="section-heading"><div><p class="eyebrow">ESCOLHAS DO ACERVO</p><h2>Séries em destaque</h2></div></div><div class="featured-grid"></div>';
  if(!pageLetter)controls.insertAdjacentElement('afterend',featured);
  ...
  function randomFeatured(){const pool=[...all];for(let i=pool.length-1;i>0;i--){const j=Math.floor(Math.random()*(i+1));[pool[i],pool[j]]=[pool[j],pool[i]]}featured.querySelector('.featured-grid').innerHTML=pool.slice(0,5).map(item=>card(item,true)).join('')}
  ...
  [['filter-platform','platform'],['filter-decade','decade'],['filter-status','status'],['filter-country','country'],['filter-sort','sort']].forEach(([id,key])=>{const el=document.getElementById(id);el.value=state[key];el.addEventListener('change',()=>{state[key]=el.value;state.limit=20;render()})});
  controls.querySelectorAll('.genre-filter input').forEach(input=>{input.checked=state.genres.includes(input.value);input.addEventListener('change',()=>{state.genres=[...controls.querySelectorAll('.genre-filter input:checked')].map(item=>item.value);state.limit=20;render()})});
  ...
  if(!pageLetter)randomFeatured();render();await import('./app.js?v=library-44');
