...
function hotdReleased(date){const[d,m,y]=date.split('/').map(Number),now=new Date(),today=new Date(now.getFullYear(),now.getMonth(),now.getDate());return new Date(y,m-1,d)<=today}
