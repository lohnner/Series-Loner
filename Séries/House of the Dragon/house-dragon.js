...
function updateHotdHeader(){const xp=(americanCount()+getHotdState().watched.length+bigBangCount()+acolyteCount()+siloCount()+stuartCount())*22,info=hotdLevelInfo(xp);document.querySelectorAll('[data-header-level]').forEach(el=>el.textContent=info.level);document.querySelectorAll('[data-header-xp]').forEach(el=>el.textContent=`${xp} XP`);document.querySelectorAll('[data-header-next]').forEach(el=>el.textContent=`${info.ceiling-xp} XP para o nível ${info.level+1}`);document.querySelectorAll('[data-header-xp-bar]').forEach(el=>el.style.width=`${info.percent}%`)}
...
const hotdAuthEntry=document.querySelector('.header-xp');if(hotdAuthEntry)hotdAuthEntry.insertAdjacentHTML('beforebegin','<a class="auth-link" href="#entrar">ENTRAR</a>');initHotd();import('../../auth-shell.js');
